...
def build_prompt(question: str, context_blocks: List[str], short_context: str, memory_blocks: List[str], memory_saved: bool = False) -> str:
    ...
    memory_note = " The user's memory request has already been saved by the system — simply confirm it was saved." if memory_saved else ""
    system = (
        "You are a careful local AI agent. Use the provided context and approved memory when useful. "
        "Only cite these label formats: [Doc N], [Web N], or [Memory N]. "
        "Do not output [System], [User], or any other bracket labels."
        f" If retrieved context is insufficient, say what is missing. Keep the answer concise.{memory_note}"
    )
    return f"""<|system|>
{system}
<|user|>
...
def execute_plan(question: str, plan: Dict[str, Any], settings: Dict[str, Any]):
    ...
    prompt = build_prompt(question, context_blocks, short_context, memory_blocks, memory_saved=memory_saved)
